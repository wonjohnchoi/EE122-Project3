...
class Firewall (object):
  ...
  def _handle_DeferredConnectionIn (self, event, flow, packet):
    ...
    if not host_name:
      if flow.dst in self.monitor_strings.keys():
        ...
        event.action.monitor_forward = True #why?
  ...
  def _handle_MonitorData (self, event, packet, reverse):
    ...
    if ip_address in self.timers.keys():
      self.timers[ip_address].cancel()

    ip_address = str(packet.payload.dstip)
    port = str(packet.payload.payload.dstport)
    if reverse:
      ip_address = str(packet.payload.srcip)
    ...
    for monitor_string in self.monitor_strings[ip_address].keys():
      data = self.prev_packet[ip_address][monitor_string] + http_data
      if self.debug: log.debug("data" + data)
      self.prev_packet[ip_address][monitor_string] = ""
      ...
      self.prev_packet[ip_address][monitor_string] += monitor_string[0]
      if httpIndex >= 0:
        for monitor_stringIndex in range(1, len(monitor_string)): #remove -1 from this line since range is not inclusive
          ...
          if data[httpIndex+monitor_stringIndex] == monitor_string[monitor_stringIndex] and monitor_stringIndex != len(monitor_string)-1: #keep negative 1 here (i'm pretty sure we didn't have it here when we were having problems, right?) because length-1 is last char in string, would explain why things weren't working before, without this it would never realize it found the entire string, so it would save the full word "bing"
            self.prev_packet[ip_address][monitor_string] += monitor_string[monitor_stringIndex]
          else:
            self.prev_packet[ip_address][monitor_string] = ""
            break    

      self.timers[ip_address] = Timer(30, self.handle_closed_connection, args=(ip_address, port))
      if self.debug: log.debug("timer created")

  def handle_closed_connection(self, ip_address, dst_port):
    ip_address = str(ip_address)
    dst_port = str(dst_port)

    if self.debug: log.debug("in connectionclos/ed")

    count = ip_address + "," + dst_port + "," + "%s,%s\n"
